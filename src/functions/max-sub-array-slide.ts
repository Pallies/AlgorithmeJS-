function maxSubArraySlide(input: number[], window: number) {
...
function maxSubArraySum(input: number[], window: number) {
  if (input.length < window) return null;
  let result: number = -Infinity;
  for (let i = 0; i + window <= input.length; i++) {
    let tmp = 0;
...
function maxSubArrayBest(input: number[], window: number) {
  let max = 0;
  let tmp = 0;
  if (input.length < window) return null;
//   calcul la somme
