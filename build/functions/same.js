...
// best solution
function sameBest(values, carre) {
    if (values.length !== carre.length)
        return false;
    //   itération
    for (let i = 0; i < values.length; i++) {
        // la valeur est dans le tableau ? -1 si non
        let index = carre.indexOf(Math.pow(values[i], 2));
        // valeur non trouvé ?
        if (index === -1)
            return false;
...
`);
// O(n)
function sameBestter(values, carre) {
    if (values.length !== carre.length)
        return false;
    //   itération
    let index = {};
    ...
    for (let key of carre)
        tableau[key] = tableau[key]++ || 0;
    for (let key in index) {
        if (index[key] !== tableau[Math.pow(parseInt(key), 2)])
            return false;
        if (!tableau.hasOwnProperty(Math.pow(parseInt(key), 2)))
            return false;
        // idem
